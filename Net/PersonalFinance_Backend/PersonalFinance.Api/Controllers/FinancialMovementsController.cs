...
namespace PersonalFinance.Api.Controllers
{
    ...
    {
        private readonly ILogger<FinancialMovementsController> _logger;
        private readonly IFinancialMovementService _service;
        private readonly IBrowserDetector _browserDetector;

        public FinancialMovementsController(ILogger<FinancialMovementsController> logger, IFinancialMovementService service, IBrowserDetector browserDetector)
        {
            _logger = logger;
            _service = service;
        ...
        [ProducesResponseType(typeof(ResponseService<FinancialMovementDto>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetByIdAsync(int id)
        {
            ...
            var response = new ResponseService<FinancialMovementDto>
            {
                Status = existResult,
        ...
        [ProducesResponseType(typeof(ResponseService<IEnumerable<FinancialMovementDto>>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetAllAsync(int? page, int? limit)
        {
            ...
            var resultDtos = result as FinancialMovementDto[] ?? result.ToArray();

            var response = new ResponseService<IEnumerable<FinancialMovementDto>>
            {
        ...
        [Produces(MediaTypeNames.Application.Json, Type = typeof(FinancialMovementModel))]
        public IActionResult Post([FromBody] FinancialMovementModel request)
        {
            _logger.LogInformation(nameof(Post));

            var objRequest = Mapper.Map<FinancialMovementDto>(request);
        ...
        [Produces(MediaTypeNames.Application.Json, Type = typeof(FinancialMovementModel))]
        public async Task<IActionResult> PutAsync(int id, [FromBody] FinancialMovementModel request)
        {
